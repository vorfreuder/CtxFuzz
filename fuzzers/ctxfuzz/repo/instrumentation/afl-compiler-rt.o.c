...
__thread u32 __afl_context_hp_op_seq;
static u32   __afl_count_ptr_initial[MAP_COUNT_SIZE];
u32         *__afl_count_ptr = __afl_count_ptr_initial;
static u8    context_hp_op_record[MAP_CTX_HP_OP_RECORD_SIZE];
static u32   recent_obj[OBJ_NUM+1];
void         CTXHTFuzzUpdateContext(void *p, u8 code) {
  register uintptr_t h = (uintptr_t)p & 0x0000FFFF;
  register u8       *record_h = context_hp_op_record + h;
  register u8        current_record = *record_h;
  current_record = (current_record << 2) | code;
  *record_h = current_record;
  // renctly used obj
  int index = 0;
  for (index = 0; index < OBJ_NUM; index++) {
    if (recent_obj[index] == h) {
      break;
    } else if (recent_obj[index] == -1) {
      recent_obj[index] = h;
      return;
    }
  }
  if (index == OBJ_NUM) { index = 0; }
  int index2 = index + 1;
  for (; index2 < OBJ_NUM && recent_obj[index2] != -1; index2++)
    recent_obj[index2 - 1] = recent_obj[index2];
  if (index2 == OBJ_NUM || recent_obj[index2] == -1) { index2--; }
  recent_obj[index2] = h;
  return;
}

void CTXHTFuzzGetContext() {
  __afl_context_hp_op_seq = 0;
  for (int i = 0; i < OBJ_NUM; i++) {
    // Consider that the type of recent_obj is unsigned int
    // so "== -1" should be instead of "< 0"
    if (recent_obj[i] == -1) break;
    __afl_context_hp_op_seq ^= context_hp_op_record[recent_obj[i]];
  }
  return;
}
...
static void __afl_start_snapshots(void) {
  ...
  /* Phone home and tell the parent that we're OK. If parent isn't there,
    ...
    memset(context_hp_op_record, 0, sizeof(context_hp_op_record));
    memset(recent_obj, -1, OBJ_NUM * sizeof(u32));
